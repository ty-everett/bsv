 ...
 * @exzmple <caption>Creates a new address from a public key:</caption>
 * const address = new Address().fromPubKey(pubKey)
 * @example <caption>Creates a new address from a string:</caption>
 * const address = new Address().fromString(string)
 * @example <caption>Converts an address to a string:</caption>
 * const string = address.toString()
 * @example <caption>Converts an address to a transaction output script:</caption>
 * const script = address.toTxOutScript()
 * @example <caption>Checks if a given string is a valid BSV address:</caption>
 * const isValid = Address.isValid(string)
 * @example <caption>Can also do testnet:</caption>
 ...
 */
export default class Address extends Struct {
  /**
   * Constructs a new Address.
   * @constructor
   * @param {number} versionByteNum The version byte number.
   * @param {Buffer} hashBuf The hash buffer.
   * @param {Object} constants The constants to use.
   */
  ...
  /** Creates a new Address from the given Buffer.
   * @param {Buffer} buf The buffer containing the Address
  */
  fromBuffer (buf) {
    if (buf.length !== 1 + 20) {
